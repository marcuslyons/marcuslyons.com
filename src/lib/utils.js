...
export const slugFromPath = (path) => path.match(/([\w-]+)\.(svelte\.md|svx)/i)?.[1] ?? null;
...
export const getSlugs = async function () {
	try {
		const posts = await import.meta.glob('../content/posts/**/*.{md,svx,svelte.md}');
...
export const getPosts = async (postsContent, body = false) => {
	const postPromises = postsContent.map(async (element) => {
		...
		const { title, excerpt, date } =
			/** @type {{title: string; excerpt: string; date: string;}} */ (transformedContent.data.fm);
