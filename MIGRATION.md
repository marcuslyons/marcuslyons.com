...
The project is on Svelte 3.54 / SvelteKit 1.5 / Vite 4 — multiple major versions behind current. The Svelte ecosystem provides official automated migration tools (`npx sv migrate`) that handle upgrades in steps: Svelte 3→4, SvelteKit 1→2, Svelte 4→5. This plan follows that recommended incremental path, verifying the build at each step.

**mdsvex verdict**: Keep it. [mdsvex 0.12.6](https://github.com/pngwn/MDsveX) has Svelte 5 support (added in 0.12.5). No better alternative exists for Svelte markdown blogs. The remark/rehype plugin versions must stay pinned to their current majors (unified v9 ecosystem) — do NOT upgrade them.
...
Svelte 4 is mainly a tooling/internals change — no syntax changes needed. The migration tool handles `package.json` version bumps.
...
2. **`package.json` scripts** — remove `--plugin-search-dir .` from lint/format (Prettier 3 removed this flag):
   ```json
...
4. **`src/routes/post/[slug]/+page.server.ts`** — `throw error(404, ...)` no longer needs `throw` (still works if left, but the tool may remove it)
...
**Note on `<svelte:component>`**: If `<data.component />` doesn't render mdsvex content correctly, fall back to keeping `<svelte:component this={data.component} />` (deprecated but still functional).
...
- `processor: 'svelte3/svelte3'` → `parser: 'svelte-eslint-parser'` with `parserOptions: { parser: '@typescript-eslint/parser' }`
...
Update `.prettierrc` to add explicit plugin registration (required by Prettier 3):
...
- Remove unused deps: `@sveltejs/adapter-auto`, `@types/cookie`, `rehype-sanitize` (in package.json but not used in mdsvex config)
...
1. `src/routes/post/[slug]/+page.svelte` — `<svelte:component>` removal + mdsvex rendering
...
mdsvex 0.12.6 uses unified v9. The current plugin versions are compatible. Do **not** upgrade these to latest majors (which require unified v11):
